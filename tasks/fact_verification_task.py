...
verification_task = Task(
    ...
    1. "sources" (Wikipedia articles): Provide all the Wikipedia articles (the sources must be selected from the search results, from the "wikipedia_article_source" field only, and must be textually cited, because will be used in future searches and must be accurate) from where you got useful information to support or refute the request_in_english and all the verification_facts. Each source must appear only once.
    2. "fragments": Fragments of Sources (Wikipedia articles) from where you got useful information to support or refute the request_in_english and all the verification_facts. This field must be textually cited, and must not be empty unless there is not enough information to support or refute the claim. The fragments can be only included if the information in it is useful to support or refute the initial input and/or one or more claims. If possible, include at least one fragment for each Wikipedia article. Exclude frgments unrelated to the clais and fact verifications.
